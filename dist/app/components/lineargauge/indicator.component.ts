...
export class IndicatorComponent {
    ...
    constructor(public chartDataService: ChartDataService) {
        this.scale = [{
            direction: 'clockwise', type: 'roundedrectangle', backgroundColor: 'transparent',
            ...
            position: { x: 52, y: 50 },
            markerPointers: [{
                value: 7000000, distanceFromScale: 20, type: 'circle', length: 125, width: 100, placement: 'center',
                backgroundColor: '#FFFFFF', opacity: 0.8, border: { color: '#8c8c8c' }
            }],
            labels: [{ font: { fontStyle: 'Normal', size: '16px', fontFamily: 'Arial' }, distanceFromScale: { x: 0, y: 30 }, placement: 'far', angle: 90 , textColor: '#8c8c8c'}],
    ...
    DrawMarkerPointers(args) {
        window['XPosition'] = args.position.startX;
        window['text'] = ((Math.round(args.pointerValue)) == 1000000) ? '1 Mill' : (Math.round(args.pointerValue) > 1000000) ?
            (Math.round(args.pointerValue) / 1000000).toFixed(3) == '1' ? '1 Mill' : (Math.round(args.pointerValue) / 1000000).toFixed(3) + ' Mills' :
            (Math.round(args.pointerValue) / 1000).toFixed(3).toString().replace('.', ',');
        let label = 'Loan Amount = $ ' + window['text'];
        $('#lbl').text(label);
    }
    DrawIndicators(args) {
        if (args.indicatorIndex == 0) {
            args.position.textLocation.x = window['XPosition'];
            args.indicatorEl.stateRanges[0].text = window['text'];
        }
    }
}
